...
// extract reads the source markdown, finds fenced blocks, determines a
...
func (c *Config) extractFromReader(reader io.Reader) error {
		...
		if strings.HasPrefix(line, c.fence) {
			...
			if len(trim) == len(c.fence) {
				if start == 0 {
					...
					if filename == "" && c.all { // generate a filename using the fence language tag.
						filename = fmt.Sprintf("code-block-%d+%d.%s", start, lineNum-start, c.matcher.lang)
					}
					if filename != "" {
						c.write(filename, buf.Bytes(), start, lineNum)
					} else {
						log.Warnf("Skip block without filename (%d lines) lang=%s %s:%d", lineNum-start, c.matcher.lang, c.mdPath, start)
					}
			...
			log.RequestPostf("Opening fence %s:%d - will consider the corresponding closing fence as part of the current code block", c.mdPath, lineNum)
			closingIsIn = true
			goto store_line
		}
	...
	if start > 0 {
		return fmt.Errorf("Unterminated fenced block starting %s:%d", c.mdPath, start)
	}

	return nil
}

// write the fenced block using the extracted filename found in the input Markdown.
// It respects dry-run and overwrite semantics and rejects any attempt
// to write outside of the output folder (directory-traversal protection).
func (c *Config) write(filename string, data []byte, start, stop int) {
