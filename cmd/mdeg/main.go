...
func defaultConfig(arguments []string) *Config {
	flags := flag.NewFlagSet("mdeg", flag.ExitOnError)
...
func main() {
	...
	if gen {
		...
		if c.count > 0 {
			log.Resultf("Generated markdown with %d code blocks: %s", c.count, c.mdPath)
		} else {
