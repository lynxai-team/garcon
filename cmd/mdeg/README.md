# mdeg – extract fenced code blocks from Markdown

`mdeg` reads a Markdown file, finds fenced code blocks
...
```bash
go run github.com/lynxai-team/garcon/cmd/mdeg@main
```

### Install

```bash
go install github.com/lynxai-team/garcon/cmd/mdeg@latest
mdeg
```
...
```txt
$ mdeg -h
mdeg - extract or generate fenced code blocks.

USAGE

  mdeg [options]      <markdown-file> [folder]
  mdeg [options] -gen [markdown-file] [folder]
...
is the folder name with ".md" extension.
  ...
  mdeg -gen folder    (generates folder.md)
...
Insert only the Go files:
  ...
  mdeg -gen -regex '[/A-Za-z0-9_-]+[.]go' src

The default header is "## File: path/file.go".
This can be changed with -header <text>.

  mdeg -gen -header "# "         =>  "# path/file.go"
  mdeg -gen -header "## File: "  =>  "## File: path/file.go"
...
mdeg README.md out/ -dry-run

# extract but never overwrite files that already exist
mdeg README.md out/ -overwrite
```
...
The `mdeg` tool can also be used as a Go library for programmatic access:

```go
import "github.com/lynxai-team/garcon/cmd/mdeg"
...
mdeg recognizes multiple patterns for extracting filename information:
...
When generating Markdown from code files, mdeg produces:
...
# run the fuzz target
go test -fuzz=FuzzExtract -run=^$ ./cmd/mdeg/
go test -fuzz=FuzzGenerate -run=^$ ./cmd/mdeg/
```
...
mdeg includes robust protection against directory traversal attacks:
...
# .githooks/pre-commit
#!/bin/bash
mdeg docs/README.md src/examples/
```
...
- name: Generate Documentation
  run: |
    go install github.com/lynxai-team/garcon/cmd/mdeg@latest
    mdeg -gen -overwrite=true src/ docs.md
```
