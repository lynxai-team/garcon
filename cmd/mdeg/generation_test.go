...
// assertFiles helper - asserts that N files exist in the directory.
func assertFiles(t *testing.T, dir string, want int) {
	t.Helper()
	...
	if len(entries) != want {
		t.Fatalf("want %d files in %s, but found %d", want, dir, len(entries))
	}
}
...
func TestRoundTrip(t *testing.T) {
	...
	for fileMD, header := range cases {
		t.Run(fileMD, func(t *testing.T) {
			...
			// Generate test
			{
				c1 := defaultConfig([]string{"-gen", "-header", header, pathMD, src})
				if c1.header != header {
					t.Fatalf("provided header=%q but got=%q", header, c1.header)
				}
				err := c1.generateMarkdown()
				if err != nil {
					t.Fatalf("generateMarkdown failed: %v", err)
				}
			}
