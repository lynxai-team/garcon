...
` + "```" + "\n"
	...
	// The filename should contain "code-block-4+3.go"
	if !strings.Contains(entries[0].Name(), "code-block-4+3.go") {
...
` + "```" + "\n"
	...
	// Should create no file
	assertFiles(t, dest, 0)

	c.all = true // Extract ALL blocks (without filename)
	err = c.extract()
	if err != nil {
		t.Fatalf("extract failed: %v", err)
	}
...
` + "```" + "\n", 1,
	...
	for _, tt := range tests {
		t.Run(tt.md[:10], func(t *testing.T) {
			...
			c.dryRun = true
			c.custom = custom
			err := c.extractFromReader(strings.NewReader(tt.md))
			if err != nil {
				t.Fatalf("Unexpected extractFromReader error %v", err)
			}
			fn := c.matcher.filename()
			if fn != "" {
				t.Fatalf("Unexpected unconsumed filename %v", fn)
			}
			if c.count != tt.numberOfExtractedFiles {
				t.Fatalf("numberOfExtractedFiles: got %d, wand %d", c.count, tt.numberOfExtractedFiles)
			}
		})
	}
}
